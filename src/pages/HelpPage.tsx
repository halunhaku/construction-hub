...
const STEPS: { title: string; steps: string[]; note?: string }[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      '在记录详情页点「导出图纸」，可选择 PNG / JPG / PDF（A4：布置图 + 一览表；双侧占路再拆成上行、下行两张布置图）保存或打印',
...
const FAQS: { q: string; a: string }[] = [
  {
    ...
    a: '布置参数超出当前模板范围，例如作业区 10-4000m、设计速度 80/100km/h、过渡区 120-200m、缓冲区 100-150m、下游与终止区至少 30m、锥桶间距 1-4m。按红色提示修改即可。',
  },
  {
  ...
  {
    q: '「导出档案」下载的是什么？',
    a: '该记录的作业区布置图、一览表（A4；双侧占路含上行/下行两张布置图）与三阶段照片的打包 ZIP，照片带施工桩号与拍摄时间水印，可用于归档报验。',
  },
  {
    q: '数据存在哪里？换手机还能看到吗？',
