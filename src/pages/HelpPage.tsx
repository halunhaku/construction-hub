...
const STEPS: { title: string; steps: string[]; note?: string }[] = [
  {
    title: '新建施工记录',
    steps: [
      '顶部「项目」进入总览，点进某个项目打开施工台账；或直接点「新建记录」',
      ...
      '点「保存，并拍摄三照」进入记录详情页',
      '按施工前、施工中、施工后三个阶段分别拍照或从相册选择，选完会立刻上传；失败可点重新上传',
    ],
  ...
  },
  {
    ...
    steps: [
      ...
      '点「导入 Excel」选中填写好的文件即可上传；表尾空行会自动跳过',
      '导入成功后会显示汇总：新增条数，以及多少条已按起止桩号生成布置图',
    ],
  ...
  },
  {
    title: '独立布控（扫码出图）',
    steps: [
      '未登录也可点顶部「布置」或扫入口二维码，填写桩号和长度后预览、导出图纸',
      '点「查看布置图」进入本机预览页（不写入系统，关掉浏览器可能丢失）',
      '未登录点「登录后保存」，登录成功后会自动写入并打开详情页；已登录则直接保存',
      '项目台账、三阶段照片、已保存的布控列表仅登录可见',
    ],
    note: '入口码指向公开布置图页。已保存布控的详情需要登录。账号由管理员配置，不开放注册。'
  },
  {
    title: '作业区布置图',
    steps: [
      '新建记录时填写布置参数；Excel 导入时能算出作业区长度的会自动带默认布置图，其余在详情页点「创建布置」',
      '各分区含义：警告区（前方预警标志）、上游过渡区（锥桶斜向导流）、缓冲区（安全净空）、作业区（施工区域）、下游过渡区（锥桶撤除）、终止区（解除限速标志）',
      '在记录详情页点「导出 PNG / JPG / PDF」（A4：布置图 + 一览表；双侧占路再拆成上行、下行两张布置图）保存或打印',
  ...
  },
  {
    ...
    steps: [
      ...
      '施工中：施工作业进行中拍摄',
      '施工后：施工完成、现场恢复后拍摄',
      '有布置图，且三个阶段各至少 1 张，记录才标记为「已完整」；否则会写明缺布置图、缺照片，或两者都缺',
  ...
  },
  {
    ...
    steps: [
      '点顶部「日历」进入「施工日历」',
      '有施工记录的日期带圆点标记：蓝色 = 有记录，绿色 = 当天全部资料完整（布置图 + 三照）',
...
const FAQS: { q: string; a: string }[] = [
  ...
  },
  {
    ...
    a: '可以直接输入新名称保存。保存后再打开新建页，下拉里就会出现这个名称。',
  },
  {
    q: '照片传不上去？',
  ...
  {
    q: '「导出档案」下载的是什么？',
    a: '该记录的作业区布置图、一览表（A4；双侧占路含上行/下行两张布置图）与三阶段照片的打包 ZIP。照片水印优先使用拍摄时间（照片 EXIF），没有则用上传时间。',
  },
  {
    q: '数据存在哪里？换手机还能看到吗？',
