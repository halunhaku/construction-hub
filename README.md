...
统一施工管理平台（`construction-hub`，当前版本 `v1.7.0`）。

现场侧管施工位置台账和施工前 / 施工中 / 施工后三阶段照片；作业区侧按高速公路养护作业习惯计算分区、画布置图，并导出 A4 图纸。线上地址：https://project.halunhaku.top

微信小程序客户端已独立到上级目录 `roadzone-control-miniprogram/`（原生小程序，单独运行，不依赖本仓库）。

## 实际能做什么

- **项目工作台**：按项目聚合施工记录，显示资料完整度（布置图 + 三阶段照片）；状态为进行中 / 资料待补充 / 已完成；支持搜索、筛选和最近访问。
- **施工台账**：高速公路 → 路段 → 桩号 → 方向（上行 / 下行）。支持单条新建、编辑，以及 Excel 批量导入。
- **三阶段影像**：每条记录按施工前、施工中、施工后上传照片（各阶段不限张数）。有布置图且三阶段各至少 1 张才算资料完整。照片上传前会压缩；打包下载时带项目名、桩号、阶段和本地拍摄时间水印。
- **作业区布置图**：在记录里创建。按警告区、上游过渡区、缓冲区、作业区、下游过渡区、终止区计算桩号范围。支持路侧或中央分隔带；中央分隔带可勾选双侧占路（上下行 180° 对称布控）。可导出 PNG / JPG / PDF（A4：布置图 + 一览表；双侧占路再拆成上行、下行两张布置图），也可和三照一起打成 ZIP 档案。
- **3D 布置出图**：未登录可打开 `/layout` 填参、预览、导出，不入库。
- **标志牌 SVG**：16 张高速公路养护作业区标志，可复制 SVG 源码。
- **施工日历**：按日查看记录；蓝点有记录，绿点当天资料完整（布置图 + 三照）。
...
| 层 | 实现 |
|---|---|
| 前端 | React 19 + Vite 8 + TypeScript，History 路径路由 |
...
migrations/          D1 增量迁移
  ...
  0004_zones.sql                 旧独立布控表（页面已去掉，库表仍在）
tests/               作业区计算与接口校验
public/              静态资源
...
导入时若同时有起始和结束桩号，且默认模板能通过校验，会按桩号长度自动写入布置图；否则 `zone_params` 为 `NULL`，需在记录里点「创建布置」。
